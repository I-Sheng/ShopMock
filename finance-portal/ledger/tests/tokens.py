...
`HR_CLIENT_ID` is here on purpose: the cross-role cases need to mint a token
that a real HR session would produce, and assert this service rejects it.
"""
import json
import time
...
HR_CLIENT_ID = 'hr-portal'
SUBJECT = '5f9a7c2e-0000-4a1b-9f3d-f1nance00001'
...
def claims_for(roles=('employee', 'finance'), **overrides):
    now = int(time.time())
    claims = {
        'exp': now + 300,
        'iat': now,
        'jti': 'a2c0f1e4-0000-0000-0000-00000000fin1',
        'iss': ISSUER,
        'aud': 'account',
        'sub': SUBJECT,
        'typ': 'Bearer',
        ...
        'preferred_username': 'finance.clerk',
        'name': 'Finance Clerk',
    }
...
def make_token(roles=('employee', 'finance'), *, key_tag='primary', headers=None,
               **overrides):
    return jwt.encode(
