"""Strict seller-dashboard access-token validation."""
import json
import os
...
def require_seller(request):
    ...
    if claims.get('azp') != 'seller-dashboard':
        raise AuthError('token was not issued to the seller-dashboard client')
    access = claims.get('resource_access')
    client = access.get('seller-dashboard') if isinstance(access, dict) else None
    roles = client.get('roles') if isinstance(client, dict) else None
    if not isinstance(roles, list) or 'seller' not in roles:
        raise ForbiddenError('seller-dashboard seller client role required')
    return claims
