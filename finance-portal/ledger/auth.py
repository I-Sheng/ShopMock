...
(oe-dashboard/ops/auth.py): the pinned RS256 *public* JWK from
`PGRST_JWT_SECRET`, which PostgREST and the Django backends already verify
against.
  ...
  5. The token must have been minted for the `finance-portal` client. A valid
     HR portal, Seller Central, storefront or IT console token is rejected even
     if it carries the finance role.
  6. Only then: the `finance` *realm* role. A client role of the same name does
     not count.

Steps 1-5 fail as 401 (the caller is not authenticated to this application);
step 6 fails as 403 (authenticated, but not Finance). That split is what makes
the cross-role behaviour legible: an HR specialist who signs in through this
portal's own client reaches step 6 and is told plainly that they are not
entitled to Finance, while a token borrowed from the HR portal never gets that
far.
"""
import json
...
class ForbiddenError(AuthError):
    """The caller is authenticated but lacks the finance realm role."""
...
def _verified_claims(token):
    if not settings.FINANCE_JWT_JWK:
        raise AuthError('token verification key is not configured')
    try:
        return jwt.decode(
            token, _verification_key(settings.FINANCE_JWT_JWK),
            algorithms=['RS256'],
            options={
...
def _check_context(claims):
    if claims.get('iss') not in settings.FINANCE_ALLOWED_ISSUERS:
...
def _check_role(claims):
    ...
    if settings.FINANCE_REQUIRED_ROLE not in roles:
        raise ForbiddenError(f'{settings.FINANCE_REQUIRED_ROLE} realm role required')
