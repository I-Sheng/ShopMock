...
| DB crash-loops with `ls: can't open '/docker-entrypoint-initdb.d/': Permission denied` | two host-side causes, both handled automatically now: (a) restrictive umask on the checkout (NETID homes: 077) — deploy.sh runs `chmod -R a+rX seed/`; (b) SELinux enforcing — the seed mounts carry the `z` relabel flag in `docker-compose.yml`. After pulling these fixes run `podman compose down -v` once (the crash loop leaves half-initialized, unseeded data volumes) and redeploy. |
