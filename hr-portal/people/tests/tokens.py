...
`FINANCE_CLIENT_ID` is here on purpose: the cross-role cases need to mint a
token that a real Finance session would produce, and assert this service
rejects it.
"""
import json
import time
...
CLIENT_ID = 'hr-portal'
FINANCE_CLIENT_ID = 'finance-portal'
SUBJECT = '5f9a7c2e-0000-4a1b-9f3d-hr0spec00001'
...
def claims_for(roles=('employee', 'hr'), **overrides):
    now = int(time.time())
    claims = {
        'exp': now + 300,
        'iat': now,
        'jti': 'a2c0f1e4-0000-0000-0000-0000000000hr',
        'iss': ISSUER,
        'aud': 'account',
        'sub': SUBJECT,
        'typ': 'Bearer',
        ...
        'preferred_username': 'hr.specialist',
        'name': 'HR Specialist',
    }
...
def make_token(roles=('employee', 'hr'), *, key_tag='primary', headers=None,
               **overrides):
    return jwt.encode(
